...
@hydra_runner(config_path="conf", config_name="zeroshot_intent_config")
def main(cfg: DictConfig) -> None:
    logging.info(f'Config Params:\n {OmegaConf.to_yaml(cfg)}')

    # initialize the model using the config file
    if cfg.pretrained_model and os.path.exists(cfg.pretrained_model):
        model = ZeroShotIntentModel.restore_from(cfg.pretrained_model)
    else:
        raise ValueError('Provide path to the pre-trained .nemo checkpoint')

    # predicting an intent of a query
    context = "paint car in blue color"
    queries = [
        'related to changing color',
        'related to help command',
        'related to car rotation',
        'related to camera position',
    ]

    pred_intents, pred_slots = model.predict_from_examples(queries, cfg.model.test_ds)
