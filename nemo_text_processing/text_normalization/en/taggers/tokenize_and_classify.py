...
class ClassifyFst(GraphFst):
    ...
    def __init__(self, input_case: str, deterministic: bool = True):
        ...
        if os.path.exists(far_file) and USE_CACHE:
            ...
            classify = (
                ...
                | pynutil.add_weight(pynini.compose(word_graph, v_word), 100)
            )

            if not deterministic:
                roman_graph = RomanFst(deterministic=deterministic).fst
                # the weight matches the word_graph weight for "I" cases in long sentences with multiple semiotic tokens
                classify |= pynutil.add_weight(pynini.compose(roman_graph, _get_verbalizer(v_roman_graph)), 100)
