...
class UNet(nn.Module):
    ...
    def forward(self, input):
        x_list, x_len = input
        x_in = x_list[0]
        # len_w_mask = x_in.shape[4]
        #make len pow 2

        x = torch.zeros(x_in.shape[0], x_in.shape[1], x_in.shape[2], 256, 2048, device=x_in.device)
        x[:, :, :, :x_in.shape[3], :x_in.shape[4]] = x_in
        ...
        out = torch.squeeze(output, 1)[:,:x_in.shape[3],:x_in.shape[4]]
        return ([out] , x_len)
